...
> {
  const { data, error } = await supabase.rpc(
    'get_public_open_play_sessions_with_availability',
  )
...
export async function getOpenPlaySession(
  ...
  const { data, error } = await supabase.rpc(
    'get_public_open_play_session_with_availability',
    {
...
export async function getMyOpenPlayParticipations(): Promise<Record<string, unknown>[]> {
  const { data, error } = await supabase.rpc(
    'get_my_open_play_participations',
  )

  if (error) {
    console.error(
      'Error fetching customer Open Play status:',
      error,
    )
    throw error
  }
...
export async function getOpenPlayPayments(
  ...
  if (error) {
    console.error(
      'Error fetching Open Play payments:',
      error,
    )
    throw error
  }
