...
import { getPaymentProofSignedUrl } from '../../services/paymentService'
...
export default function PendingPayments() {
  ...
  const [resolvedProofs, setResolvedProofs] =
    ...
    async function resolveProofs() {
      ...
      const entries = await Promise.all(
        paths.map(async (path) => {
          try {
            return [path, await getPaymentProofSignedUrl(path)] as const
          } catch {
            return null
          }
        })
      ...
      {filteredBookings.length > 0 && (
        <div className="space-y-4">
          {filteredBookings.map((booking) => {
            ...
            const paymentProof = paymentProofPath
              ? resolvedProofs[paymentProofPath] ?? null
              : null
