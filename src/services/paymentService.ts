...
export async function uploadPaymentProof(
  ...
  const fileName = `${organizationId}/${idPrefix}-${Date.now()}.${fileExt}`
...
/** Resolve a payment-proof object path to a short-lived signed URL.
 * Legacy public URLs are returned unchanged for backward compatibility.
 */
export async function getPaymentProofSignedUrl(
  proofPathOrUrl: string,
  expiresInSeconds = 300,
): Promise<string> {
  if (!proofPathOrUrl) return proofPathOrUrl

  if (/^https?:\/\//i.test(proofPathOrUrl)) {
    return proofPathOrUrl
  }

  const { data, error } = await supabase.storage
    .from('payment-proofs')
    .createSignedUrl(proofPathOrUrl, expiresInSeconds)
