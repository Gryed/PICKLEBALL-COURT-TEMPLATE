...
interface Stats {
  ...
  openPlayToday: number
  openPlayPending: number
}
...
const navItems = [
  ...
  },
  {
    to: '/admin/open-play',
    label: 'Open Play',
    desc: 'Manage sessions, participants, and Open Play payments.',
    icon: '👥',
  },
  {
    to: '/admin/reports',
  ...
  async function loadStats() {
    try {
      ...
      setStats({
        ...
        openPlayToday: openPlayTodayRes.count ?? 0,
        openPlayPending: openPlayPendingRes.count ?? 0,
      })
    } catch (error) {
      console.error(error)
    } finally {
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <section className="mb-7">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5 sm:gap-4">
            ...
            <StatCard
              icon="👥"
              label="Open Play Today"
              value={loading ? '—' : stats.openPlayToday}
              description="Open Play sessions scheduled today"
            />

            <StatCard
              icon="⏳"
              label="Open Play Pending"
              value={loading ? '—' : stats.openPlayPending}
              description="Participants awaiting payment review"
              accent
            />
          </div>
        </section>
